...
1. EC2 + Nginx → Crash simulated via bash script.
2. CloudWatch Alarm → Detects downtime.
3. EventBridge Rule → Triggers Lambda.
4. Lambda Function → Orchestrates remediation + AI analysis.
5. SSM Command → Restarts Nginx service.
6. Bedrock AI → Generates human-readable incident report.
7. S3 + SNS → Stores report and sends Discord alert.
