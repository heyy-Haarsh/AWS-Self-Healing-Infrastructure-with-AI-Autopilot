...
# ── START ─────────────────────────────────────────────────────
Sep
Log "SELF-HEALING PIPELINE - AUTOMATED TEST STARTED (v2)"
Sep
...
# FIX v2: capture stderr but do NOT suppress it — let the output file be written
$lambdaErr = aws lambda invoke `
...
if (Test-Path $lambdaOut) {
    ...
    $statusCode = 0
    Log "  [FAIL] Output file not created. AWS CLI error output:" "ERROR"
    Log "  $lambdaErr" "ERROR"
}
...
if ($s3List -match "incidents/") {
    ...
    if ($latestKey) {
        Log "  Fetching latest report content..."
...
if ($pending -eq "true") {
    ...
    Log "         Click the link in the AWS Notification - Subscription Confirmation email." "WARN"
} else {
...
# FIX v2: write JSON body to a file, then use file:// so PowerShell braces don't get mangled
$bedrockBodyFile = "bedrock_body.json"
$bedrockOutFile  = "bedrock_test.json"
[System.IO.File]::WriteAllText(
    (Resolve-Path "." ).Path + "\$bedrockBodyFile",
    '{"anthropic_version":"bedrock-2023-05-31","max_tokens":20,"messages":[{"role":"user","content":"Reply with the single word: OK"}]}'
)

$bedrockErr = aws bedrock-runtime invoke-model `
    --model-id "anthropic.claude-3-haiku-20240307-v1:0" `
    --body "file://$bedrockBodyFile" `
    --content-type "application/json" `
    --accept "application/json" `
    --region $REGION `
    $bedrockOutFile 2>&1

if ($LASTEXITCODE -eq 0 -and (Test-Path $bedrockOutFile)) {
    $reply = (Get-Content $bedrockOutFile -Raw | ConvertFrom-Json).content[0].text
    Log "  [PASS] Bedrock invocation SUCCEEDED."
    Log "  Bedrock reply: '$reply'"
    $bedrockPass = $true
} else {
    Log "  [FAIL] Bedrock invocation FAILED." "ERROR"
    Log "  Error: $bedrockErr" "ERROR"
...
Log ("  SNS email:          " + $(if ($pending -ne 'true')          {'[PASS]'} else {'[WARN] Confirm email first'}))
Log ("  Bedrock invoke:     " + $(if ($bedrockPass)                 {'[PASS]'} else {'[FAIL] Enable Claude 3 Haiku in AWS Console'}))
Sep
